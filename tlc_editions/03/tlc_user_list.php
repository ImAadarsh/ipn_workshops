...
switch ($type) {
    case 'total_registrations':
        $sql = "SELECT id, name, email, mobile, institute_name, city, is_tlc_new_edition_3, tlc_join_date_edition_3, tlc_email_sent_edition_3 
                FROM users 
                WHERE tlc_edition_3 = 1 
                ORDER BY id DESC";
        break;
    ...
    case 'new_users':
        $sql = "SELECT id, name, email, mobile, institute_name, city, is_tlc_new_edition_3, tlc_join_date_edition_3, tlc_email_sent_edition_3 
                FROM users 
                WHERE tlc_edition_3 = 1 AND is_tlc_new_edition_3 = 1 
                ORDER BY id DESC";
        break;
    ...
    case 'old_users':
        $sql = "SELECT id, name, email, mobile, institute_name, city, is_tlc_new_edition_3, tlc_join_date_edition_3, tlc_email_sent_edition_3 
                FROM users 
                WHERE tlc_edition_3 = 1 AND (is_tlc_new_edition_3 = 0 OR is_tlc_new_edition_3 IS NULL) 
                ORDER BY id DESC";
        break;
            ...
            <tbody>
                <?php foreach ($users as $user): ?>
                    <tr>
                        ...
                        <td>
                            <?php if ($user['is_tlc_new_edition_3'] == 1): ?>
                         ...
                         <?php endif; ?>
                        <td><?php echo $user['tlc_join_date_edition_3'] ? date('d M Y', strtotime($user['tlc_join_date_edition_3'])) : 'N/A'; ?></td>
                        <td>
                            <?php if ($user['tlc_email_sent_edition_3'] == 1): ?>
...
if (isset($_GET['export']) && $_GET['export'] == 'csv') {
    ...
    foreach ($users as $user) {
        $row = [
            ...
            $user['city'] ?: 'N/A',
            $user['is_tlc_new_edition_3'] == 1 ? 'New' : 'Existing'
        ];
        ...
        $row[] = $user['tlc_join_date_edition_3'] ? date('d M Y', strtotime($user['tlc_join_date_edition_3'])) : 'N/A';
        $row[] = $user['tlc_email_sent_edition_3'] == 1 ? 'Yes' : 'No';
